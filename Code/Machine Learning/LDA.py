...
def feature_sel(x_train, y_train):
    x_train_lda, x_test_lda, y_train_lda, y_test_lda = train_test_split(x_train, y_train, test_size=0.2,random_state=123)
    forward_fs_best = sfs(estimator=LDA(), n_features_to_select='auto')
    forward_fs_best.fit(x_train_lda, np.ravel(y_train_lda))
    sfs_indices=forward_fs_best.support_
    x_train_svc = x_train_lda.loc[:, sfs_indices]
    fs_sel_feal = x_train_svc.columns
    return fs_sel_feal

def lda_model(int_state,poss_state):
    x_train, x_test, y_train, y_test = prepare_data(int_state, poss_state)
    sel_feat = feature_sel(x_train, y_train)
    lda_model=LDA(shrinkage = 'auto')
    lda_model.fit(x_train[sel_feat],np.ravel(y_train))
    return lda_model
...
def lda_assess(int_state, poss_state):
    # Prepare Data
    x_train,x_test,y_train,y_test=prepare_data(int_state,poss_state)
    ...
    # Model Building
    lda_model1=lda_model(int_state,poss_state)
    # Model Evaluation
    y_pre_prob = lda_model1.predict_proba(x_test[sel_features])
    ...
    def main():
        int_state = 1
        poss_state = [-1, 0, 1, 2]
        lda_assess(int_state, poss_state)
