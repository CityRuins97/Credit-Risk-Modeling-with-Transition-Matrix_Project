...
def prepare_data(initial_state,possible_state,file_path='ProcessedFile/'):
    ...
    merged_real = pd.concat([x_train, y_train], axis=1)
    merged_real = merged_real.dropna()
    y_train = merged_real[target]
    x_train = merged_real.drop(target, axis=1)
    merged_real = pd.concat([x_test, y_test], axis=1)
    merged_real = merged_real.dropna()
    y_test = merged_real[target]
    x_test = merged_real.drop(target, axis=1)
